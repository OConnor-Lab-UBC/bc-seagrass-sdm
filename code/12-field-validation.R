...
roc_metrics <- map_dfr(pred_cols, function(p) {
  ...
  # pull CV threshold (threshold_spatial) for that model
  ...
  obs  <- df_nointertidal$Presence
  pred <- df_nointertidal[[p]]
  
  keep <- !is.na(obs) & !is.na(pred)
  obs  <- obs[keep]
  pred <- pred[keep]
  
  # if Presence is stored as factor/character, uncomment:
  # obs <- as.numeric(as.character(obs))
  
  # ROC on field-validation data
  rocobj <- roc(obs, pred, quiet = TRUE)
  
  # best threshold/TSS from field-validation data itself
  coordsdf <- coords(
    rocobj,
    x = "all",
    ret = c("threshold", "sensitivity", "specificity"),
  ...
  # field-validation metrics using the CV-derived threshold
...
cliffs_field <- model_comparison_no_intertidal %>%
  transmute(
    model = case_when(
...
roc_metrics <- map_dfr(pred_cols, function(p) {
  ...
  # pull CV threshold (threshold_spatial) for that model
  cv_threshold <- combined_metrics_surfgrass %>%
    filter(model == model_name) %>%
    pull(threshold_spatial)
