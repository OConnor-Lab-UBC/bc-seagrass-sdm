...
dat_surf <- filter(seagrass_data_long, species == sp) %>% rename(fold = fold_seagrass, inner_fold = innerfold_seagrass)
...
### nested spatial cross validation to choose hyper parameters and report on metrics
gbm_surf_bccm <- nested_gbm_spatial(
  dat = dat_surf,
  predictors = pred_vars_bccm,
  outer_col = "fold",
  inner_col = "inner_fold",
...
save(gbm_surf_bccm, file = "code/output_data/model_results/surfgrass_eval_cv_gbm_bccm.RData")
gbm_surf_bccm$fold_metrics %>%
  dplyr::count(interaction.depth, shrinkage, n.minobsinnode, n.trees, sort = TRUE)
gbm_surf_bccm$fold_metrics %>%
  dplyr::count(interaction.depth, shrinkage, n.minobsinnode, sort = TRUE)
gbm_surf_bccm$fold_metrics %>%
...
xgb_surf_bccm <- nested_xgb_spatial(
  dat = dat_surf,
  predictors = pred_vars_bccm,
  outer_col = "fold",
  inner_col = "inner_fold",
...
save(xgb_surf_bccm, file = "code/output_data/model_results/surfgrass_eval_cv_xgb_bccm.RData")

xgb_surf_bccm$fold_metrics %>%
  dplyr::count(eta, max_depth, subsample, colsample_bytree, nrounds, sort = TRUE)
xgb_surf_bccm$fold_metrics %>%
  dplyr::group_by(eta, max_depth, subsample, colsample_bytree, nrounds) %>%
  dplyr::summarise(
...
gbm_surf_nep <- nested_gbm_spatial(
  dat = dat_surf,
  predictors = pred_vars_nep,
  outer_col = "fold",
  inner_col = "inner_fold",
...
save(gbm_surf_nep, file = "code/output_data/model_results/surfgrass_eval_cv_gbm_nep.RData")

gbm_surf_nep$fold_metrics %>%
  dplyr::count(interaction.depth, shrinkage, n.minobsinnode, n.trees, sort = TRUE)
gbm_surf_nep$fold_metrics %>%
  dplyr::count(interaction.depth, shrinkage, n.minobsinnode, sort = TRUE)
gbm_surf_nep$fold_metrics %>%
...
xgb_surf_nep <- nested_xgb_spatial(
  dat = dat_surf,
  predictors = pred_vars_nep,
  outer_col = "fold",
  inner_col = "inner_fold",
...
save(xgb_surf_nep, file = "code/output_data/model_results/surfgrass_eval_cv_xgb_nep.RData")
xgb_surf_nep$fold_metrics %>%
  dplyr::count(eta, max_depth, subsample, colsample_bytree, nrounds, sort = TRUE)
xgb_surf_nep$fold_metrics %>%
  dplyr::group_by(eta, max_depth, subsample, colsample_bytree, nrounds) %>%
  dplyr::summarise(
...
data_pre2013_surf <- dat_surf %>%
  filter(Year < 2010) %>%
  mutate(row_id = row_number())
names(data_pre2013_surf)

unique(data_pre2013_surf$Survey) # can't include survey because only RSU, Cuke and GDK in early years
test_set <- dat_surf %>% filter(Year > 2012)
obs_test <- test_set$presence


# based off most chosen parameters UPDATE THESE
best_gbm_bccm <- list(
  n.trees = 5000,
  interaction.depth = 4,
  shrinkage = 0.005,
...
forecast_gbm_bccm_surfgrass <- run_temporal_gbm_forecast(
  data_pre2013 = data_pre2013_surf,
  test_set = test_set,
  predictors = pred_vars_bccm,
  threshold = cv_thr_gbm_bccm$threshold,
  response = "presence",
...
forecast_gbm_nep_surfgrass <- run_temporal_gbm_forecast(
  data_pre2013 = data_pre2013_surf,
  test_set = test_set,
  predictors = pred_vars_nep,
  threshold = cv_thr_gbm_nep$threshold,
...
forecast_xgb_bccm_surfgrass <- run_temporal_xgb_forecast(
  data_pre2013 = data_pre2013_surf,
  test_set = test_set,
  predictors = pred_vars_bccm,
  threshold = cv_thr_xgb_bccm$threshold,
...
forecast_xgb_nep_surfgrass <- run_temporal_xgb_forecast(
  data_pre2013 = data_pre2013_surf,
  test_set = test_set,
  predictors = pred_vars_nep,
  threshold = cv_thr_xgb_nep$threshold,
...
forecast_ml_surfgrass <- bind_rows(
  forecast_gbm_bccm_surfgrass,
  forecast_gbm_nep_surfgrass,
  forecast_xgb_bccm_surfgrass,
  forecast_xgb_nep_surfgrass
)

forecast_summary_ml_surfgrass <- forecast_ml_surfgrass %>%
  dplyr::filter(dataset == "forecast") %>%
  dplyr::group_by(model) %>%
  dplyr::summarise(
    AUC = mean(AUC, na.rm = TRUE),
...
save(forecast_ml_surfgrass, forecast_summary_ml_surfgrass, file = "code/output_data/model_results/forecast_ml_surfgrass_models.RData")
